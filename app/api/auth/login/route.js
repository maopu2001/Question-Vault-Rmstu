...
export async function POST(req) {
  try {
    ...
    const token = await jwtSign(payload, process.env.JWT_SECRET, { expirationTime: '1y' });
    ...
    const role = await jwtSign({ role: auth.role }, process.env.NEXT_PUBLIC_JWT_SECRET, { expirationTime: '1y' });
