...
export async function GET(req) {
  ...
  try {
    ...
    const newToken = await jwtSign(newPayload, process.env.JWT_SECRET, { expirationTime: '1y' });
    ...
    await jwtSign({ role: auth.role }, process.env.NEXT_PUBLIC_JWT_SECRET, { expirationTime: '1y' });
